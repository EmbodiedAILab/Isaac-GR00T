...
class WebsocketPolicyServer:
    ...
    async def preprocess_observation_single_panda(self, raw_input: Dict[str, Any]) -> Dict[str, Any]:
        """
        Input
        {
            "state": np.ndarray(shape=(17,)), 
        ...
        state = raw_input["state"].astype(np.float32)
        processed.update({
            "state.end_effector_position_relative": state[:3].reshape(1,3),
            "state.end_effector_rotation_relative": state[3:7].reshape(1,4),
            "state.gripper_qpos": state[7:10].reshape(1,3),
            "state.base_position": state[10:13].reshape(1,3),
            "state.base_rotation": state[13:17].reshape(1,4),
        })
    ...
    async def merge_actions(self, action_dict):
        base_motion = action_dict['action.base_motion']
        control_mode = action_dict['action.control_mode']
        end_effector_position = action_dict['action.end_effector_position']
        end_effector_rotation = action_dict['action.end_effector_rotation']
        gripper_close = action_dict['action.gripper_close']

        if gripper_close.ndim == 1:
            gripper_close = gripper_close.reshape(-1, 1)

        if control_mode.ndim == 1:
            control_mode = control_mode.reshape(-1, 1)

        merged_actions = np.concatenate(
            [base_motion, control_mode, end_effector_position, end_effector_rotation, gripper_close],
            axis=1
        )
    ...
    async def _handler(self, websocket: websockets.asyncio.server.ServerConnection):
        ...
        while True:
            try:
                ...
                obs = await self.preprocess_observation_single_panda(obs)
                # print(f"obs: {obs}")
